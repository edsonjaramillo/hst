...
REPO="${HST_REPO:-edsonjaramillo/hst}"
VERSION="${HST_VERSION:-latest}"
INSTALL_DIR="${HST_INSTALL_DIR:-$HOME/.local/bin}"
VERIFY_CHECKSUM=1

usage() {
  cat <<'USAGE'
Install hst from GitHub Releases.

Usage:
  install.sh [--version vX.Y.Z] [--install-dir PATH] [--no-verify]

Environment overrides:
  HST_REPO         GitHub repo in owner/name format (default: edsonjaramillo/hst)
  HST_VERSION      Version tag like v0.2.0 or "latest" (default: latest)
  HST_INSTALL_DIR  Install directory (default: ~/.local/bin)
USAGE
}
...
archive_name="hst_${version_no_v}_${os}_${arch}.tar.gz"
...
binary_path="${tmp_dir}/extract/hst_${version_no_v}_${os}_${arch}/hst"
...
mkdir -p "${INSTALL_DIR}"
install -m 0755 "${binary_path}" "${INSTALL_DIR}/hst"

echo "hst installed to ${INSTALL_DIR}/hst"
echo "Run: ${INSTALL_DIR}/hst --help"
